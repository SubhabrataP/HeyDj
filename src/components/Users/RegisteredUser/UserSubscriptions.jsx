...
export default class UserSubscriptions extends Component {
  ...
  getFilteredPlaylists = () => {
    ...
    this.state.currentSubscriptions.map((elem) => {
      if (
        this.state.allPlaylists.find((x) => x.id === elem.playlistId) !==
        undefined
      ) {
        this.state.currentPlaylists.push(
          this.state.allPlaylists.find((x) => x.id === elem.playlistId)
        );
        current.push(
          this.state.allPlaylists.find((x) => x.id === elem.playlistId)
    ...
    this.state.upcomingSubscriptions.map((elem) => {
      if (
        ...
        this.state.upcomingPlaylists.push(
          this.state.allPlaylists.find((x) => x.id === elem.playlistId)
        );
      }
    });
    ...
    this.state.expiredSubscriptions.map((elem) => {
      if (
        ...
        this.state.expiredPlayLists.push(
          this.state.allPlaylists.find((x) => x.id === elem.playlistId)
        );
      }
    });
  ...
  generatePlaylist = (tracks) => {
    const playlist = tracks.map((item) => {
      return {
        file: item.content,
  ...
  onTracksClick = (data) => {
    // console.log(data)
    let x = this.state.currentSubscriptions.find(
      (x) => x.playlistId === data.id
    );

    apiAxios
      .get("api/subscription/" + x.id, {
        headers: {
          Authorization: localStorage.getItem("Token"),
        },
      })
      .then((res) => {
        ...
        const playlist = this.generatePlaylist(res.data.contents);
  ...
  onImageClick = (item, type) => {
    let x = this.state.upcomingSubscriptions.find(
  ...
  playAll = () => {
    ...
    const playlist = this.state.selectedPlaylistTracks.map((item) => {
      return {
        file: item.content,
        image: item.thumbnail,
        tracks: [],
    ...
    let promise = new Promise((resolve, rej) => {
      this.setState({ showTracksModal: false, playlist }, () =>
        this.setOpen(true, this.sidebarRef)
      );
      resolve();
    });
  ...
  setOpen = (isOpen) => {
    console.log(isOpen, this.sidebarRef, "setopen ");
    if (this.sidebarRef) {
      if (isOpen == false) {
        // console.log("ref", ref);
        this.sidebarRef.style.right = "-40vw";
      } else {
        // console.log("ref", ref);
        this.sidebarRef.style.right = "0px";
      }
    ...
    return (
      <Layout history={this.props.history}>
                  ...
                  {this.state.upcomingPlaylists.map((item, index) => {
                    return (
                      <div
                        ...
                        style={{ color: "#fff", marginBottom: "15px" }}
                        onClick={() => {
                          this.onImageClick(item, "upcoming");
                        }}
                      >
                        <img
                          src={item.thumbnail}
        ...
        <SubscribedTracks
          ...
          contents={
            this.state.type === "current"
              ? this.state.selectedPlaylistTracks
              : this.state.selectedPlaylistDetails
          }
          onDismiss={() => {
            this.dismissTracks();
          }}
        ...
        <PlayerContainer
          ...
          playNextMusic={this.playNextMusic}
