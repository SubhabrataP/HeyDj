...
export default class AddEditPlaylist extends Component {
            ...
            if (this.props.isAdd) {
                ...
                if (this.state.selectedContent.length > 0) {
                    ...
                    if (this.state.sampleContent.name === "") {
                        apiAxios.put("/api/dj/playlist/" + this.state.editPlaylistId,
                            bodyFormData, {
                            headers: {
                                'Authorization': localStorage.getItem('Token')
                            }
                        ...
                        apiAxios.put("/api/dj/playlist/" + this.state.editPlaylistId,
                            bodyFormData, {
                            params: {
                                'sampleType': this.state.sampleContentType
                            },
                            headers: {
                                'Authorization': localStorage.getItem('Token')
                            }
    ...
    addSampleContentUrl = (url, alertMessage) => {
        axios.put(url, this.state.sampleContentBlob)
            .then((res) => {
                console.log("res", res)
                this.setState({
                            ...
                            <React.Fragment>
                                ...
                                <div className="container loginBg">
                                    ...
                                    <div className="row mt-2 mb-2">
                                        ...
                                        <div className="col-md-12" style={{ color: 'black', paddingLeft: "7%" }}>
                                            {this.state.sampleContent.name === "" ? "" :
                                                <div className="row">
                                                    <Label style={{ textAlign: "center", color: '#fff' }}>{this.state.sampleContent.name}</Label>
                                        ...
                                        {this.props.isAdd ? "" :
                                            this.state.sampleContent.name === "" ?
                                                <React.Fragment>
                                                    {this.state.sampleContentType === "audio" ?
                                                        <audio className="ml-3" style={{ marginLeft: "20%" }} src={this.state.sampleContent.value} controls></audio>
                                                        :
                                                        <video style={{ height: "60%", width: "60%", marginLeft: "20%" }} src={this.state.sampleContent.value} controls ></video>}
                                                    <button className="customBtn" style={{ marginLeft: "20px" }} onClick={() => { this.fileChange.click(); }}>Change</button>
                                                </React.Fragment> : null
                                        }
                                    </div>
